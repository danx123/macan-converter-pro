...
# --- [BARU] Kamus untuk Teks Multi-Bahasa ---
LANGUAGES = {
    "id": {
        # Judul & Tab
        ...
        "modes": ["PDF ke Gambar", "PNG ke ICO"],
...
bersih dan mudah digunakan."""
    ...
    "en": {
        ...
        # Common UI
        "input_label": "1. Select Input File:",
...
class FileConversionWorker(QObject):
    ...
    def run(self):
        try:
            if self.mode in ["PDF ke Gambar", "PDF to Image"]:
                self._convert_pdf_to_image()
            elif self.mode in ["PNG ke ICO", "PNG to ICO"]:
                self._convert_png_to_ico()
            else:
                self.conversion_error.emit(f"Unknown conversion mode: {self.mode}")
...
class AudioConversionWorker(QObject):
    ...
    def run(self):
        base_name = os.path.splitext(os.path.basename(self.input_path))[0]
        output_filename = os.path.join(self.output_path, f"{base_name}.{self.out_format}")
        command = [self.ffmpeg_path, '-i', self.input_path, '-vn']
...
class ConverterApp(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # --- [BARU] Header dengan Judul dan Pilihan Bahasa ---
        ...
        self.about_tab = self._create_about_tab() # [BARU]
        ...
        self.tab_widget.addTab(self.about_tab, self.lang["tab_about"]) # [BARU]

    # --- [BARU] Logika Multi-Bahasa ---
    ...
    def _retranslate_ui(self):
        ...
        self.tab_widget.setTabText(4, self.lang["tab_about"])
        ...
        self._update_image_input_ui() # Mengupdate placeholder juga
        ...
        self.img_quality_combo.setCurrentText(self.lang["img_qualities"][2]) # Baik/Good
        ...
        self.vid_quality_combo.setCurrentText(self.lang["vid_qualities"][1]) # Sedang/Medium
    ...
    def _create_about_tab(self):
        ...
        self.about_text.setHtml(self.lang["about_content"])
        self.about_text.setObjectName("aboutText") # Untuk styling
    ...
    def browse_input_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, self.lang["input_label"], "", filter)
        if file_path:
    ...
    def _update_ui_for_mode(self, mode):
        is_pdf_mode = mode == self.lang["modes"][0]
    ...
    def browse_image_input_file(self):
        ...
        if is_batch:
            files, _ = QFileDialog.getOpenFileNames(self, self.lang["input_img_label"], "", filter)
            if files:
                self.image_batch_files = files
                self.img_input_path_edit.setText(f"{len(files)} file(s) selected.")
        else:
            file_path, _ = QFileDialog.getOpenFileName(self, self.lang["input_img_label"], "", filter)
            if file_path:
    ...
    def browse_video_input_file(self):
        ...
        if is_batch:
            files, _ = QFileDialog.getOpenFileNames(self, self.lang["input_vid_label"], "", filter)
            if files:
                self.video_batch_files = files
                self.vid_input_path_edit.setText(f"{len(files)} file(s) selected.")
        else:
            file_path, _ = QFileDialog.getOpenFileName(self, self.lang["input_vid_label"], "", filter)
            if file_path:
    ...
    def browse_audio_input_file(self):
        ...
        if is_batch:
            files, _ = QFileDialog.getOpenFileNames(self, self.lang["input_audio_label"], "", filter)
            if files:
                self.audio_batch_files = files
                self.audio_input_path_edit.setText(f"{len(files)} file(s) selected.")
        else:
            file_path, _ = QFileDialog.getOpenFileName(self, self.lang["input_audio_label"], "", filter)
            if file_path:
    ...
    def conversion_finished(self, button, progress_bar, status_label, message):
        ...
        output_path = ""
        if button == self.convert_btn: output_path = self.output_path_edit.text()
        elif button == self.img_convert_btn: output_path = self.img_output_path_edit.text()
        elif button == self.vid_convert_btn: output_path = self.vid_output_path_edit.text()
        elif button == self.audio_convert_btn: output_path = self.audio_output_path_edit.text()
