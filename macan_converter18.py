...
import re
import subprocess  # [DIpertahankan] untuk _open_output_folder
...
# [MODIFIKASI] Mengimpor QProcess dan QSettings
from PySide6.QtCore import (
    QThread, QObject, Signal, Qt, QSize, QByteArray, QProcess, QSettings
)
...
# [MODIFIKASI] Menggunakan QProcess untuk VideoConversionWorker
class VideoConversionWorker(QObject):
    ...
    def _get_media_duration(self):
        command_args = ['-i', self.input_path]
        process = QProcess()
        process.start(self.ffmpeg_path, command_args)
        
        # Beri waktu 5 detik untuk mendapatkan info, lalu kill jika macet
        if not process.waitForFinished(5000):
            process.kill()
            return None
        
        # ffmpeg sering mencetak info durasi ke stderr
        ...
        duration_search = self.duration_regex.search(output)
        if duration_search:
    ...
    def run(self):
        ...
        try:
            ...
            # [FIX] Hubungkan ke stderr untuk progres ffmpeg
            ...
            if not self.process.waitForStarted():
                self.conversion_error.emit(self.lang["ffmpeg_failed_to_start"])
                return
                
            # 'run' selesai, tapi QProcess berjalan di event loop thread ini

        except Exception as e:
            if self.is_running: self.conversion_error.emit(self.lang["error_during_conversion"].format(error=str(e)))

    def _read_progress(self):
        if not self.process or not self.total_duration or not self.is_running:
            return
            
        output = bytes(self.process.readAllStandardError()).decode('utf-8', 'ignore')
        
        # [FIX] Tangani output \r dari ffmpeg untuk progres
        last_line = ""
        if '\r' in output:
        ...
        if last_line:
            ...
            if time_search:
                ...
                if current_time > self.total_duration: # Handle jika durasi terdeteksi salah
    ...
    def _on_process_finished(self, exit_code):
        if not self.is_running:
            ...
            self.conversion_error.emit(self.lang["ffmpeg_failed_with_code"].format(code=exit_code, error=error_msg))
...
# [MODIFIKASI] Menggunakan QProcess untuk AudioConversionWorker
class AudioConversionWorker(QObject):
    ...
    def run(self):
        base_name = os.path.splitext(os.path.basename(self.input_path))[0]
        output_filename = os.path.join(self.output_path, f"{base_name}.{self.out_format}")
        command_args = ['-i', self.input_path, '-vn', '-b:a', self.bitrate.split(' ')[0] + 'k', '-y', output_filename]
    ...
    def _read_progress(self):
        ...
        if last_line:
            ...
            if time_search:
                ...
                progress = int((current_time / self.total_duration) * 100)
                progress = min(progress, 100) # Pastikan tidak lebih dari 100
                self.progress_updated.emit(progress, self.lang["converting_progress"].format(progress=progress))
...
class RedesignedConverterApp(QMainWindow):
    def __init__(self):
        ...
        # [MODIFIKASI] Setup QSettings
        # Ini harus dilakukan sebelum UI dibuat agar default dapat dibaca
        self.settings = QSettings("MacanAngkasa", "MacanConverterPro")
        ...
        # [MODIFIKASI] Muat pengaturan setelah UI dibuat
        self._load_settings()
    ...
    def browse_master_output_folder(self):
        ...
        # Update QLineEdit di semua halaman
    ...
    def _update_ui_for_mode(self, mode):
        ...
        placeholder_key = "input_placeholder_multi_img" if is_image_to_pdf else "input_placeholder_file"
        placeholder = self.lang.get(placeholder_key, "Pilih file...")
    ...
    def start_file_conversion(self):
        ...
        if (is_multi_file_mode and not input_path) or (not is_multi_file_mode and not os.path.exists(input_path)):
    ...
    def _run_worker(self):
        ...
        # [MODIFIKASI] Pastikan thread dibersihkan setelah selesai
        
        # HAPUS DUA BARIS PENYEBAB ERROR INI:
        # self.worker.finished.connect(self.thread.quit)
        # self.worker.finished.connect(self.worker.deleteLater)
        
        # Baris ini sudah benar dan akan membersihkan thread
        # setelah self.thread.quit() dipanggil di slot lain.
    ...
    def on_conversion_finished(self, msg):
        ...
        if is_batch_active:
            ...
            if self.thread: 
                ...
                self.worker.deleteLater() # <-- TAMBAHKAN INI
            self._start_next_batch_conversion()
        else:
    ...
    def on_conversion_error(self, msg):
        ...
        # Tampilkan error untuk file batch saat ini
        is_batch_active = hasattr(self, 'current_mode_key') and self.batch_files[self.current_mode_key]
        if is_batch_active:
             file_list = self.batch_files[self.current_mode_key]
             error_on_file = self.lang["error_in_batch"].format(index=self.current_batch_index + 1, error=msg)
             QMessageBox.critical(self, self.lang["error_title"], error_on_file)
             # Hentikan sisa batch
             self.batch_files[self.current_mode_key] = []
             self.current_batch_index = 0
        else:
    ...
    # --- [MODIFIKASI] Fungsi Simpan & Muat Pengaturan ---
    def _save_settings(self):
        self.settings.setValue("language", self.lang_combo.currentText())
        self.settings.setValue("lastCategory", self.category_list.currentRow())
        self.settings.setValue("shutdownOnFinish", self.shutdown_checkbox.isChecked())
        
        # Simpan folder output (cukup satu, karena tombol master menyamakan semua)
        self.settings.setValue("masterOutputPath", self.vid_output_path_edit.text())

        # Video
        self.settings.setValue("vid/batch", self.batch_mode_checkbox.isChecked())
        self.settings.setValue("vid/format", self.vid_format_combo.currentText())
        self.settings.setValue("vid/resolution", self.vid_resolution_combo.currentText())
        self.settings.setValue("vid/quality", self.vid_quality_combo.currentText())
        
        # Audio
        self.settings.setValue("audio/batch", self.audio_batch_mode_checkbox.isChecked())
        self.settings.setValue("audio/format", self.audio_format_combo.currentText())
        self.settings.setValue("audio/bitrate", self.audio_bitrate_combo.currentText())

        # Ekstrak Audio
        self.settings.setValue("va/batch", self.va_batch_mode_checkbox.isChecked())
        self.settings.setValue("va/format", self.va_format_combo.currentText())
        self.settings.setValue("va/bitrate", self.va_bitrate_combo.currentText())

        # Gambar
        self.settings.setValue("img/batch", self.img_batch_mode_checkbox.isChecked())
        self.settings.setValue("img/format", self.img_format_combo.currentText())
        self.settings.setValue("img/resolution", self.img_resolution_combo.currentText())
        self.settings.setValue("img/quality", self.img_quality_combo.currentText())
        
        # Dokumen
        self.settings.setValue("doc/mode", self.mode_combo.currentText())
        self.settings.setValue("doc/format", self.format_combo.currentText())
        
        self.settings.beginGroup("doc/ico_sizes")
        for size, cb in self.ico_checkboxes.items():
            self.settings.setValue(size, cb.isChecked())
        self.settings.endGroup()

    def _load_settings(self):
        # Muat bahasa
        lang = self.settings.value("language", "Indonesia", type=str)
        self.lang_combo.setCurrentText(lang)
        # Memuat bahasa akan memicu _retranslate_ui, jadi UI akan diperbarui
        
        # Muat folder output utama
        master_path = self.settings.value("masterOutputPath", "", type=str)
        self.vid_output_path_edit.setText(master_path)
        self.audio_output_path_edit.setText(master_path)
        self.va_output_path_edit.setText(master_path)
        self.img_output_path_edit.setText(master_path)
        self.output_path_edit.setText(master_path)
        
        # Umum
        self.shutdown_checkbox.setChecked(self.settings.value("shutdownOnFinish", False, type=bool))

        # Video
        self.batch_mode_checkbox.setChecked(self.settings.value("vid/batch", False, type=bool))
        self.vid_format_combo.setCurrentText(self.settings.value("vid/format", "mp4", type=str))
        self.vid_resolution_combo.setCurrentText(self.settings.value("vid/resolution", "Original Size", type=str))
        self.vid_quality_combo.setCurrentText(self.settings.value("vid/quality", self.lang["vid_qualities"][1], type=str))

        # Audio
        self.audio_batch_mode_checkbox.setChecked(self.settings.value("audio/batch", False, type=bool))
        self.audio_format_combo.setCurrentText(self.settings.value("audio/format", "mp3", type=str))
        self.audio_bitrate_combo.setCurrentText(self.settings.value("audio/bitrate", "192 kbps", type=str))

        # Ekstrak Audio
        self.va_batch_mode_checkbox.setChecked(self.settings.value("va/batch", False, type=bool))
        self.va_format_combo.setCurrentText(self.settings.value("va/format", "mp3", type=str))
        self.va_bitrate_combo.setCurrentText(self.settings.value("audio/bitrate", "192 kbps", type=str))

        # Gambar
        self.img_batch_mode_checkbox.setChecked(self.settings.value("img/batch", False, type=bool))
        self.img_format_combo.setCurrentText(self.settings.value("img/format", "JPG", type=str))
        self.img_resolution_combo.setCurrentText(self.settings.value("img/resolution", "Original Size", type=str))
        self.img_quality_combo.setCurrentText(self.settings.value("img/quality", self.lang["img_qualities"][2], type=str))
        self._update_image_options() # Panggil untuk menyetel visibilitas Kualitas

        # Dokumen
        self.mode_combo.setCurrentText(self.settings.value("doc/mode", self.lang["modes"][0], type=str))
        self.format_combo.setCurrentText(self.settings.value("doc/format", "PNG", type=str))
        self._update_ui_for_mode(self.mode_combo.currentText()) # Panggil untuk menyetel visibilitas UI

        self.settings.beginGroup("doc/ico_sizes")
        for size, cb in self.ico_checkboxes.items():
            cb.setChecked(self.settings.value(size, True, type=bool))
        self.settings.endGroup()
        
        # Muat kategori terakhir DILAKUKAN PALING AKHIR
        last_category = self.settings.value("lastCategory", 0, type=int)
        # Pastikan tidak di luar jangkauan jika kategori berubah
        if last_category >= self.category_list.count():
            last_category = 0
        self.category_list.setCurrentRow(last_category)
    ...
    # [MODIFIKASI] Simpan pengaturan saat menutup
    def closeEvent(self, event):
        self._save_settings() # Simpan pengaturan
