...
# Class ini bertugas untuk menjalankan proses berat (konversi) di background
# agar aplikasi utama (GUI) tidak freeze.
class Worker(QObject):
    ...
    def run(self):
        try:
            ...
            # Mengirimkan traceback untuk debugging yang lebih detail
            import traceback
...
class MacanConverterApp(QWidget):
    ...
    def setup_styles(self):
        # Modern dark theme using QSS (like CSS)
        self.setStyleSheet("""
            QWidget {
    ...
    def start_html_conversion(self):
        ...
        # HTML conversion with QWebEngineView must run in the main thread.
        # We don't use the worker here, but manage it directly.
    ...
    # --- Conversion Logic (Tasks for Worker) ---
    def convert_pdf_task(self, pdf_path, out_dir, fmt):
        # FIX 1: Create QPdfDocument without a parent. The 'self' from the original code
        # referred to the main window, which is in a different thread, causing an error.
        doc = QPdfDocument()
        doc.load(pdf_path)
        ...
        for i in range(page_count):
            # FIX 2: The 'render' method does not take a 'dpi' keyword argument.
            # Instead, we calculate the target image size in pixels based on desired DPI.
            dpi = 150
            # Get the page size in points (1 point = 1/72 inch)
            page_size_points = doc.pageSizeF(i)
            # Calculate the target size in pixels: (width_in_inches * dpi)
            target_size = (page_size_points * (dpi / 72.0)).toSize()
            
            # Render the page to a QImage with the calculated dimensions.
            ...
            self.worker.progress.emit(progress_val)
    
    def convert_html_task(self, html_path, output_path):
        # This part runs in the main thread due to GUI component (QWebEngineView)
        # We will manage its lifecycle carefully
        self.web_view = QWebEngineView()
        # The view doesn't need to be visible to the user to render
    ...
    def on_html_load_finished(self, success):
        # This is a slot that gets called when the page is loaded
        if success:
            # Using a QTimer to give the page a moment to render any final JS
    ...
    def capture_html_page(self):
        try:
            ...
            if not pixmap.save(self.html_output_path_temp):
                 self.show_error("Failed to save the image.")
            else:
                 # Manually call the success functions since we are not using a worker
                 self.status_bar.showMessage("Conversion successful!", 5000)
