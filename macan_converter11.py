...
# --- [BARU] Kelas Worker untuk Proses Konversi Gambar di Latar Belakang ---
class ImageConversionWorker(QObject):
    progress_updated = pyqtSignal(int, str)
    conversion_finished = pyqtSignal(str)
    conversion_error = pyqtSignal(str)

    def __init__(self, input_path, output_path, out_format, resolution, quality_str):
        super().__init__()
        self.input_path = input_path
        self.output_path = output_path
        self.out_format = out_format
    ...
    def run(self):
        try:
            self.progress_updated.emit(0, f"Membuka {os.path.basename(self.input_path)}...")
            ...
            if self.resolution != "Original Size":
                try:
                    # Ambil angka dari string seperti "1920x1080" atau "2560x1440 (2K)"
                    res_parts = self.resolution.split(' ')[0].split('x')
                    new_size = (int(res_parts[0]), int(res_parts[1]))
                    img = img.resize(new_size, Image.Resampling.LANCZOS)
                except (ValueError, IndexError):
                    self.conversion_error.emit(f"Format resolusi tidak valid: {self.resolution}")
                    return

            self.progress_updated.emit(50, "Memproses gambar...")
            if not self.is_running: return
            ...
            # Jika gambar punya alpha channel (transparansi) dan akan disimpan ke JPG
            if self.out_format.lower() in ['jpeg', 'jpg']:
                if img.mode in ('RGBA', 'LA', 'P'):
                    img = img.convert('RGB')

            img.save(output_filename, **save_options)
            self.progress_updated.emit(100, "Gambar berhasil disimpan.")
            self.conversion_finished.emit(f"Sukses! Gambar telah dikonversi ke {self.out_format.upper()}.")
...
class ConverterApp(QMainWindow):
    def __init__(self):
        ...
        # Atribut untuk batch mode
        self.video_batch_files = []
        self.current_batch_index = 0
        self.audio_batch_files = []
        self.current_audio_batch_index = 0
        self.image_batch_files = [] # [BARU]
        self.current_image_batch_index = 0 # [BARU]
    ...
    def _setup_ui(self):
        ...
        image_converter_tab = self._create_image_converter_tab() # [BARU]
        ...
        tab_widget.addTab(image_converter_tab, "Image Converter") # [BARU]
    ...
    def _create_image_converter_tab(self):
        """[BARU] Membuat widget untuk Tab Image Converter."""
        tab_widget = QWidget()
        layout = QVBoxLayout(tab_widget)
        layout.setContentsMargins(10, 20, 10, 10)
        layout.setSpacing(15)
        ...
        img_io_layout.addWidget(QLabel("1. Pilih File Gambar Input:"))
        img_input_layout = QHBoxLayout()
        self.img_input_path_edit = QLineEdit()
        self.img_input_path_edit.setReadOnly(True)
        self.img_input_path_edit.setPlaceholderText("Pilih satu atau beberapa file gambar...")
        img_browse_btn = QPushButton("Browse...")
        img_browse_btn.clicked.connect(self.browse_image_input_file)
        img_input_layout.addWidget(self.img_input_path_edit)
        img_input_layout.addWidget(img_browse_btn)
        img_io_layout.addLayout(img_input_layout)

        img_io_layout.addWidget(QLabel("2. Pilih Folder Output:"))
        img_output_layout = QHBoxLayout()
        self.img_output_path_edit = QLineEdit()
        self.img_output_path_edit.setPlaceholderText("Pilih folder tujuan...")
        self.img_output_path_edit.setReadOnly(True)
        img_browse_output_btn = QPushButton("Browse...")
        img_browse_output_btn.clicked.connect(self.browse_image_output_folder)
        img_output_layout.addWidget(self.img_output_path_edit)
        img_output_layout.addWidget(img_browse_output_btn)
        img_io_layout.addLayout(img_output_layout)

        self.img_batch_mode_checkbox = QCheckBox("Batch Mode (Konversi Banyak File Sekaligus)")
        ...
        self.img_quality_label = QLabel("Kualitas:")
        res_quality_layout.addWidget(self.img_quality_label)
        self.img_quality_combo = QComboBox()
        self.img_quality_combo.addItems(["Maksimum (100)", "Sangat Baik (95)", "Baik (85)", "Sedang (75)", "Rendah (50)"])
        self.img_quality_combo.setCurrentText("Baik (85)") 
        ...
        # --- Tombol Aksi dan Status ---
        self.img_convert_btn = QPushButton("Mulai Konversi Gambar")
        ...
        self.img_status_label = QLabel("Siap untuk mengonversi gambar.")
        self.img_status_label.setObjectName("statusLabel")
        layout.addWidget(self.img_status_label)

        layout.addStretch()
        self._update_image_options() # Panggil sekali untuk set state awal
        return tab_widget
    ...
    # --- [BARU] Logika untuk Image Converter ---
    def _update_image_input_ui(self):
        """Mengupdate UI input gambar berdasarkan status batch mode."""
        is_batch = self.img_batch_mode_checkbox.isChecked()
        if is_batch:
            self.img_input_path_edit.setPlaceholderText("Pilih beberapa file gambar untuk konversi batch...")
        else:
            self.img_input_path_edit.setPlaceholderText("Pilih satu file gambar...")
        self.img_input_path_edit.clear()
        self.image_batch_files = []

    def _update_image_options(self):
        """Menyembunyikan/menampilkan opsi kualitas berdasarkan format output."""
    ...
    def browse_image_input_file(self):
        """Membuka file dialog untuk satu atau banyak file gambar."""
        is_batch = self.img_batch_mode_checkbox.isChecked()
        filter = "Image Files (*.png *.jpg *.jpeg *.bmp *.webp *.gif)"
        
        if is_batch:
            files, _ = QFileDialog.getOpenFileNames(self, "Pilih File-file Gambar", "", filter)
            if files:
                self.image_batch_files = files
                self.img_input_path_edit.setText(f"{len(files)} file gambar dipilih.")
        else:
            file_path, _ = QFileDialog.getOpenFileName(self, "Pilih File Gambar", "", filter)
            if file_path:
                self.img_input_path_edit.setText(file_path)

    def browse_image_output_folder(self):
        folder_path = QFileDialog.getExistingDirectory(self, "Pilih Folder Output Gambar")
        if folder_path:
            self.img_output_path_edit.setText(folder_path)

    def start_image_conversion(self):
        """Memulai proses konversi gambar untuk mode tunggal atau batch."""
        output_path = self.img_output_path_edit.text()
        if not output_path or not os.path.isdir(output_path):
            QMessageBox.warning(self, "Output Tidak Valid", "Silakan pilih folder output gambar yang valid.")
            return

        is_batch = self.img_batch_mode_checkbox.isChecked()
        if is_batch:
            if not self.image_batch_files:
                QMessageBox.warning(self, "Input Tidak Valid", "Silakan pilih setidaknya satu file gambar untuk mode batch.")
                return
            ...
            if not input_path or not os.path.exists(input_path):
                QMessageBox.warning(self, "Input Tidak Valid", "Silakan pilih file gambar input yang valid.")
                return
            self._start_single_image_conversion(input_path)

    def _start_single_image_conversion(self, input_path):
        """Helper untuk memulai konversi satu file gambar."""
        self.img_convert_btn.setEnabled(False)
        self.img_progress_bar.setValue(0)
        self.img_status_label.setText(f"Mempersiapkan konversi untuk {os.path.basename(input_path)}...")
        ...
        self.thread = QThread()
        self.worker = ImageConversionWorker(input_path, output_path, out_format, resolution, quality)
        self.worker.moveToThread(self.thread)
    ...
    def _start_next_batch_image_conversion(self):
        ...
        if self.current_image_batch_index < len(self.image_batch_files):
            ...
            self.img_status_label.setText(
                f"Mengonversi file {self.current_image_batch_index + 1} dari {len(self.image_batch_files)}: {os.path.basename(input_path)}"
            )
            self._start_single_image_conversion(input_path)
        else:
            final_msg = f"Batch selesai! Semua {len(self.image_batch_files)} gambar telah berhasil dikonversi."
    ...
    def on_image_conversion_error(self, msg):
        is_batch = self.img_batch_mode_checkbox.isChecked()
        if is_batch and self.image_batch_files:
            error_msg = f"Error pada file {self.current_image_batch_index + 1}: {msg}\n\nProses batch dihentikan."
    ...
    def conversion_finished(self, button, progress_bar, status_label, message):
        ...
        if button == self.convert_btn:
            output_path = self.output_path_edit.text()
        elif button == self.img_convert_btn: # [BARU]
            output_path = self.img_output_path_edit.text()
