...
# [MODIFIKASI] Mengimpor library baru yang dibutuhkan
try:
    import docx
    ...
    print("PERINGATAN: Beberapa fungsionalitas mungkin tidak berfungsi. Pastikan Anda menginstal library yang dibutuhkan dengan 'pip install python-docx docx2pdf openpyxl'")
...
class FileConversionWorker(QObject):
    ...
    # [MODIFIKASI] Fungsi konversi baru
    ...
    def _convert_pdf_to_xlsx(self):
        ...
        for i, page in enumerate(doc):
            ...
            if not tables.tables:
                ...
                for r, block in enumerate(text_blocks, 1):
                    sheet.cell(row=r, column=1, value=block[4])
            else:
                for table_num, table in enumerate(tables, 1):
                    for r_idx, row in enumerate(table.extract(), 1):
                        for c_idx, cell_text in enumerate(row, 1):
                            sheet.cell(row=r_idx, column=c_idx, value=cell_text)
            progress = int(((i + 1) / total_pages) * 100)
            self.progress_updated.emit(progress, self.lang["converting_page"].format(current=i+1, total=total_pages))
    ...
    def _convert_image_to_pdf(self):
        # self.input_path adalah list of files di sini
        ...
        for i, img_path in enumerate(self.input_path):
            ...
            try:
                ...
                img_doc.close()
                img_pdf.close()
            except Exception: # Fallback untuk format gambar non-standar
                img = Image.open(img_path).convert("RGB")
                img.save("temp_img.png")
                img_doc = fitz.open("temp_img.png")
                pdf_bytes = img_doc.convert_to_pdf()
                img_pdf = fitz.open("pdf", pdf_bytes)
                doc.insert_pdf(img_pdf)
                img_doc.close()
            ...
            status_text = f"Menambahkan gambar {i+1} dari {total_files}..."
            self.progress_updated.emit(progress, status_text)
...
class ConverterApp(QMainWindow):
    ...
    def browse_input_file(self):
        ...
        filters = {
            ...
            self.lang["modes"][6]: "Image Files (*.png *.jpg *.bmp)", # Gambar ke PDF
        }
