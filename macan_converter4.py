...
class Worker(QObject):
    ...
    def run(self):
        try:
            # PERBAIKAN 1: Kirim instance worker (self) ke dalam fungsi target
            # agar bisa digunakan sebagai parent QObject di thread yang benar.
...
class MacanConverterApp(QWidget):
    ...
    # ... (Tidak ada perubahan di UI creation)
    ...
    # PERBAIKAN 2: Tambahkan 'worker_ref' sebagai parameter pertama.
    def convert_pdf_task(self, worker_ref, pdf_path, out_dir, fmt):
        # PERBAIKAN 3: Gunakan 'worker_ref' sebagai parent dari QPdfDocument.
